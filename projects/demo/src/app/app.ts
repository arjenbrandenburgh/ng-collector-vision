...
export class App {
  ...
  protected readonly totalCount = computed(() =>
    this.cards().reduce((n, c) => n + c.quantity, 0),
  );
  ...
  protected onCardDetected(detection: CardDetection): void {
    ...
    const placeholder: EnrichedCard = {
      ...
      name: detection.cardId, setName: '', setCode: '',
      thumbnailUrl: null, loading: true,
    };
    this.cards.update(list => [placeholder, ...list]);
    this.#triggerFlash(detection.cardId);

    this.#scryfall.lookup(detection.cardId)
  ...
  protected onScannerClosed(): void { this.scannerOpen.set(false); }
  protected openScanner():     void { this.scannerOpen.set(true); }

  // ── Card list mutations ───────────────────────────────────────────────────

  protected increment(card: EnrichedCard): void {
    this.cards.update(list =>
      list.map(c => c.detection.cardId === card.detection.cardId
  ...
  protected decrement(card: EnrichedCard): void {
    this.cards.update(list =>
      list
        .map(c => c.detection.cardId === card.detection.cardId
          ? { ...c, quantity: c.quantity - 1 } : c)
  ...
  protected remove(card: EnrichedCard): void {
    this.cards.update(list =>
      list.filter(c => c.detection.cardId !== card.detection.cardId),
    );
  }

  protected clearAll(): void { this.cards.set([]); }

  #triggerFlash(cardId: string): void {
    this.recentId.set(cardId);
    setTimeout(() => { if (this.recentId() === cardId) this.recentId.set(null); }, 800);
  }
}
